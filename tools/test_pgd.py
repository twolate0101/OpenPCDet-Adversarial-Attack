...
与 demo.py 不同，PGD 攻击需要访问模型以计算梯度，
因此注入点在 collate_batch/load_data_to_gpu 之后、model.forward 之前。

攻击完成后自动输出对比报告并保存并排 BEV 图。

用法:
    cd /root/OpenPCDet/tools
    python test_pgd.py --cfg_file cfgs/kitti_models/pointpillar.yaml \
        --ckpt ../ckpts/pointpillar_7728.pth \
        --data_path ../data/kitti/testing/velodyne/000008.bin \
        --severity 0.5 --iterations 10
"""
import argparse
import glob
import os
...
def parse_config():
    parser = argparse.ArgumentParser(description='PGD Attack Test')
    ...
    parser.add_argument('--severity', type=float, default=0.5, help='PGD epsilon (m)')
    parser.add_argument('--iterations', type=int, default=10, help='PGD iterations')
...
def print_comparison(before, after):
    print(f'\n{"="*62}')
    print(f'  PGD Attack Comparison Report')
    print(f'{"="*62}')
...
def main():
    ...
    logger.info('-----------------PGD Attack Test-------------------------')
    logger.info(f'Epsilon={args.severity}m  Iterations={args.iterations}')
    ...
    for idx, data_dict in enumerate(demo_dataset):
        ...
        draw_bev(axes[1], pts, pred_adv['pred_boxes'].cpu().numpy(),
                 labels_adv.cpu().numpy(), f'ADVERSARIAL  ({stats_after["num"]} objects)')
        plt.tight_layout(pad=2)
        save_path = 'compare_bev.png'
        plt.savefig(save_path, dpi=200, bbox_inches='tight', facecolor=fig.get_facecolor())
        logger.info(f'Comparison BEV saved to: {os.path.abspath(save_path)}')

    logger.info('PGD attack test done.')
